# React AI Sources
URL: /ai/sources
Expandable source citations like Perplexity's "Used 5 sources". React component for transparent AI with TypeScript and shadcn/ui styling.

***

title: React AI Sources
description: Expandable source citations like Perplexity's "Used 5 sources". React component for transparent AI with TypeScript and shadcn/ui styling.
icon: BookOpen
component: true
---------------
...
<Accordions type="single">
  ...
  <Accordion id="source-extraction" title="How do I extract sources from the AI response?">
    Sources come as separate parts in the message in React applications. Look for
    parts with type "source-url" or check the message.sources array in JavaScript
    implementations.
  </Accordion>
  ...
  <Accordion id="custom-icons" title="Can I use different icons for sources?">
    Pass custom children to the Source component to override the default book icon
    and layout in TypeScript components.
  </Accordion>

  {" "}

  <Accordion id="source-grouping" title="Should I group sources by type or domain?">
    Consider categorizing sources (research, documentation, news) for better
    organization when you have many sources in React applications.
  </Accordion>
